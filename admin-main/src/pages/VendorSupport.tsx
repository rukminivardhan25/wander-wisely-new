...
import { HelpCircle, MessageSquare, Eye } from "lucide-react";
...
export function VendorSupport() {
  ...
  const [viewRow, setViewRow] = useState<TicketRow | null>(null);

  const load = async () => {
    setLoading(true);
    setError(null);
    try {
      const data = await vendorHubFetch<{ tickets: TicketRow[] }>("/api/admin/support-tickets");
      setTickets(data.tickets ?? []);
    } catch (e) {
      setError(e instanceof Error ? e.message : "Failed to load vendor support tickets");
      setTickets([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleSendReply = async () => {
    ...
    setSubmitting(true);
    try {
      await vendorHubFetch(`/api/admin/support-tickets/${replyingId}/reply`, {
        method: "PATCH",
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-foreground">Vendor Support</h1>
        <p className="text-muted-foreground mt-1">
          Support tickets submitted by vendors from the Partner Portal Support page. Review and reply here.
        </p>
      </div>
      ...
      <div className="bg-card rounded-2xl border border-forest-200 shadow-card overflow-hidden">
        <div className="px-6 py-4 border-b border-forest-200 flex items-center gap-2">
          <HelpCircle size={20} className="text-forest-600" />
          <span className="font-semibold text-foreground">All tickets</span>
        </div>
        {loading ? (
              ...
              <table className="w-full">
                <thead>
                  <tr className="bg-forest-50 border-b border-forest-200">
                    ...
                    <th className="text-left text-xs font-medium text-muted-foreground uppercase tracking-wider px-6 py-3.5">Subject</th>
                ...
                <tbody className="divide-y divide-forest-200">
                  ...
                  {tickets.map((row) => (
                    <tr key={row.id} className="hover:bg-forest-50/50 transition-colors">
                      ...
                      <td className="px-6 py-3.5 text-sm text-foreground max-w-[200px] truncate" title={row.subject}>
                        {row.subject}
                      </td>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-card rounded-2xl border border-forest-200 shadow-lg max-w-md w-full p-6">
            ...
            <textarea
              ...
              placeholder="Your reply to the vendor..."
      ...
      {viewRow !== null && (
        ...
        >
          ...
          >
            <h3 className="font-semibold text-foreground mb-1">{viewRow.subject}</h3>
            <p className="text-xs text-muted-foreground mb-3">
              {viewRow.vendorName} · {viewRow.vendorEmail} · {formatDate(viewRow.createdAt)}
            </p>
            <h4 className="text-xs font-medium text-muted-foreground uppercase tracking-wider mb-1">Message</h4>
