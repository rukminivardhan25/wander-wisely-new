...
router.post("/", async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const bookedCount = parseInt(booked.rows[0]?.sum ?? "0", 10);
    if (bookedCount + participantsCount > slot.rows[0].capacity) {
    ...
    await query(
      `INSERT INTO experience_bookings (booking_ref, experience_id, experience_slot_id, user_id, participants_count, total_cents, status)
       VALUES ($1, $2, $3, $4, $5, $6, 'confirmed')`,
      [ref, experienceId, experienceSlotId, userId, participantsCount, totalCents]
    );

    const row = await query<{ id: string; booking_ref: string; created_at: string }>(
...
/** GET /api/experience-bookings — List current user's experience bookings */
...
router.get("/:id", async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const row = await query<{
      ...
      `SELECT b.id, b.booking_ref, b.experience_id, b.experience_slot_id, b.user_id, b.participants_count, b.total_cents, b.status, b.paid_at::text, b.created_at::text,
       s.slot_date::text AS slot_date, s.slot_time::text AS slot_time,
       e.name AS exp_name, e.city AS exp_city, e.duration_text AS exp_duration, e.location_address AS exp_location
       FROM experience_bookings b
       JOIN experience_slots s ON s.id = b.experience_slot_id
       JOIN experiences e ON e.id = b.experience_id
    ...
    res.json({
      ...
      experienceName: r.exp_name,
      experienceCity: r.exp_city,
      durationText: r.exp_duration,
      locationAddress: r.exp_location ?? undefined,
    });
  } catch (err) {
...
router.patch("/:id/pay", async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    await query(
      "UPDATE experience_bookings SET paid_at = now(), updated_at = now() WHERE id = $1 AND user_id = $2",
      [id, userId]
    );
    ...
    const row = await query<{
      ...
      `SELECT b.id, b.booking_ref, s.slot_date::text AS slot_date, s.slot_time::text AS slot_time,
       e.name AS exp_name, e.city AS exp_city, b.participants_count, b.total_cents, b.paid_at::text
       FROM experience_bookings b
       JOIN experience_slots s ON s.id = b.experience_slot_id
       JOIN experiences e ON e.id = b.experience_id
       WHERE b.id = $1 AND b.user_id = $2`,
